...
namespace CVScreeningService.Tests.UnitTest.Dispatching
{
    ...
    public class Assignement
    {
        ...
        private ScreeningLevelDTO _screeningLevel2;
        private ScreeningLevelVersionDTO _screeningLevelVersion1;
        private ScreeningLevelVersionDTO _screeningLevelVersion2;
        ...
        [Test]
        public void Retrieve_Assignement_Screeners_When_Nothing_Is_Assigned()
        {
            ...
            var assignement = _dispatchingManagementService.GetScreenerAtomicCheckAssignmentWeight(educationAtomicCheck);

            Assert.AreEqual(0, assignement.ElementAt(0).Value);
            Assert.AreEqual(0, assignement.ElementAt(1).Value);
            Assert.AreEqual(0, assignement.ElementAt(2).Value);
            Assert.AreEqual(0, assignement.ElementAt(3).Value);
            Assert.AreEqual(0, assignement.ElementAt(4).Value);
            Assert.AreEqual(0, assignement.ElementAt(5).Value);
            Assert.AreEqual(0, assignement.ElementAt(6).Value);
            Assert.AreEqual(0, assignement.ElementAt(7).Value);
            Assert.AreEqual(0, assignement.ElementAt(8).Value);
        }
        
        [Test]
        public void Retrieve_Assignement_Screeners_When_Already_Assigned()
        {
            var screeningDTO1 = CreateQualifyAndAssignScreeningForScreenerOffice1AndScreenerOnField1();

            var screeningDTO2 = CreateAndQualifyScreening("Screening qualified");
            var atomicChecks = _screeningService.GetAllAtomicChecksForScreening(screeningDTO2);
            ...
            Assert.AreEqual(100, assignement.ElementAt(0).Value);
            Assert.AreEqual(0, assignement.ElementAt(1).Value);
            Assert.AreEqual(0, assignement.ElementAt(2).Value);
            Assert.AreEqual(0, assignement.ElementAt(3).Value);
            Assert.AreEqual(0, assignement.ElementAt(4).Value);
            ...
            var policeAtomicCheck = atomicChecks.First(
                u => u.TypeOfCheck.TypeOfCheckCode == (Byte)TypeOfCheckEnum.POLICE_CHECK);

            assignement = _dispatchingManagementService.GetScreenerAtomicCheckAssignmentWeight(policeAtomicCheck);

            Assert.AreEqual(0, assignement.ElementAt(0).Value);
            Assert.AreEqual(0, assignement.ElementAt(1).Value);
            Assert.AreEqual(100, assignement.ElementAt(2).Value);
            Assert.AreEqual(0, assignement.ElementAt(3).Value);
            Assert.AreEqual(0, assignement.ElementAt(4).Value);
            Assert.AreEqual(0, assignement.ElementAt(5).Value);
            Assert.AreEqual(0, assignement.ElementAt(6).Value);
        ...
        public void Retrieve_Assignement_Screeners_When_Atomic_Check_Does_Not_Have_Qualification_Place()
        {
            var screeningDTO = CreateAndQualifyScreening();
            var atomicChecks = _screeningService.GetAllAtomicChecksForScreening(screeningDTO);

            var mediaCheck = atomicChecks.First(
                u => u.TypeOfCheck.TypeOfCheckCode == (Byte)TypeOfCheckEnum.MEDIA_CHECK_COMPREHENSIVE);

            var assignement = _dispatchingManagementService.GetScreenerAtomicCheckAssignmentWeight(mediaCheck);
        ...
        private void InitializeTest()
        {
            ...
            _screeningLevel2 = Utilities.BuildScreeningLevelDTO();
            _screeningLevel2.ScreeningLevelName = "2nd screening level";
            ...
            _screeningLevelVersion1.TypeOfCheckScreeningLevelVersion.Add(new TypeOfCheckScreeningLevelVersionDTO
            {
                TypeOfCheck = new TypeOfCheckDTO
                {
                    CheckName = "Media check comprehensive",
                    TypeOfCheckCode = (Byte) TypeOfCheckEnum.MEDIA_CHECK_COMPREHENSIVE
                },
            ...
            _screeningLevelVersion2 = Utilities.BuildScreeningLevelVersionDTO();
            _screeningLevelVersion2.TypeOfCheckScreeningLevelVersion = Utilities.BuildTypeOfCheckList2ForScreeningListDTO();
            _screeningLevelVersion2.ScreeningLevelVersionTurnaroundTime = 5;

            error = _clientService.CreateScreeningLevel(_clientContract1, ref _screeningLevel2, ref _screeningLevelVersion2);
            Assert.AreEqual(ErrorCode.NO_ERROR, error);
        ...
        /// </summary>
        /// <returns></returns>
        private ScreeningDTO CreateScreening(string screeningName = null)
        {
            // Create screening
            var screeningDTO = Utilities.BuildScreeningDTO(_screeningLevelVersion1, _qualityControl1);
            if (screeningName != null)
                screeningDTO.ScreeningFullName = screeningName;
        ...
        /// </summary>
        /// <returns></returns>
        private ScreeningDTO CreateAndQualifyScreening(string screeningName = null)
        {
            // Create screening
            var screeningDTO = CreateScreening(screeningName);
            var screening = _unitOfWork.ScreeningRepository.Single(u => u.ScreeningId == screeningDTO.ScreeningId);
        ...
        /// </summary>
        /// <returns></returns>
        private ScreeningDTO CreateQualifyAndAssignScreeningEqually(string screeningName = null)
        {
            var error = ErrorCode.NO_ERROR;

            // Create screening
            var screeningDTO = CreateAndQualifyScreening(screeningName);
            var screening = _unitOfWork.ScreeningRepository.Single(u => u.ScreeningId == screeningDTO.ScreeningId);
